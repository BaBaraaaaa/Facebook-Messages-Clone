...
// Lấy conversations theo userId
  ...
  const handleFriendChange = (e: React.ChangeEvent<HTMLInputElement>) => setFriendValue(e.target.value);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', px: 2, pt: 2, pb: 1 }}>
          <Tooltip title="Thêm bạn để trò chuyện">
            <IconButton color="primary" onClick={handleDialogOpen} sx={{ bgcolor: '#f5f6fa', borderRadius: 2, boxShadow: 1, '&:hover': { bgcolor: '#e3eafc' } }}>
              <PersonAddIcon />
            </IconButton>
          </Tooltip>
        </Box>
        <List sx={{ flex: 1, overflowY: "auto", p: 0 }}>
          {list.length === 0 ? (
            ...
            list.map((item) => {
              ...
              const otherUser: import("../../types/user").IUser | undefined = !isGroup
                ? item.members.find(m => m._id !== user?._id)
                : undefined;
              const userId = user?._id || '';
              const unread = userId && item.unreadCounts ? item.unreadCounts[userId] || 0 : 0;
              const isActive = item._id === activeId;
              return (
                <ListItem disablePadding key={item._id} sx={{ mx: 1, my: 0.5, borderRadius: 2 }}>
                  <ListItemButton
                  ...
                  >
                    <ListItemAvatar >
                      <Avatar src={isGroup ? undefined : otherUser?.avatarUrl}>
                        {isGroup ? (item.name?.[0] || 'G') : (otherUser ? otherUser.name?.[0] : '?')}
                      </Avatar>
                    </ListItemAvatar>
                    <ListItemText
                      primary={
                    ...
                    />
                    {unread > 0 && (
                      <Box sx={{ ml: 1, minWidth: 24, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Box sx={{ bgcolor: 'primary.main', color: '#fff', borderRadius: '50%', px: 1, fontSize: 12, fontWeight: 700 }}>
                          {unread}
                        </Box>
                      </Box>
                    )}
        ...
        <Dialog open={openDialog} onClose={handleDialogClose} maxWidth="xs" fullWidth>
          ...
          <DialogActions>
            ...
            <Button onClick={handleAddFriend} variant="contained" disabled={!friendValue.trim()}>
              Thêm bạn
            </Button>
          </DialogActions>
        </Dialog>
